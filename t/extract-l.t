...
BEGIN {
  ...
  plan skip_all => 'Text::CSV is missing'      unless eval 'require Text::CSV;1';
...
plan skip_all => "ack: $!" unless open my $ACK, '-|', q[ack '\bl\('];
my %lexicons;
while (<$ACK>) {
  ...
  local $" = ', ';
  ok $n <= int(@f), "parse $text ==> (@f)" if $ENV{BUILD_ASSETS} > 1 or $n > int @f;
}
